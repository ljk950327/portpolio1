    ...
    <%@ include file="top.jsp" %>
      ...
      <c:forEach var="list" items="${requestScope.list }">
        <div class="col-sm-6 col-lg-6 col-md-6">
          <div class="thumbnail">
            ...
            <div class="caption">
              ...
              <h3>
                <a href="Content.me?num=${list.num }">${list.goodsname }</a>
              </h3>
              <p>${list.content }</p>
            </div>
          </div>
        ...
        <c:if test="${requestScope.startPage>3 }">
          <li class="page-item">
            <a class="page-link" href="List.me?pg=${requestScope.startPage -1 }&gk=1" aria-label="Previous"> <span aria-hidden="true">«</span> <span class="sr-only">Previous</span> </a>
          </li>
        </c:if>
        <c:forEach var="i" begin="${startPage }" end="${endPage }" step="1"	>
          <c:if test="${i==pg }">
          <li class="page-item">
            <a class="page-link" href="List.me?pg=${requestScope.startPage}&gk=1">${i }</a>
          </li>
          </c:if>
          <c:if test="${i!=pg }">
          <li class="page-item">
            <a class="page-link" href="List.me?pg=${i}&gk=1">${i }</a>
          </li>
          </c:if>
          </c:forEach>
          <c:if test="${requestScope.endPage<totalPage }">
          <li class="page-item">
            <a class="page-link" href="List.me?pg=${requestScope.endPage +1 }&gk=1" aria-label="Next"> <span aria-hidden="true">»</span> <span class="sr-only">Next</span> </a>
          </li>
          </c:if>
        </ul>
      </div>
